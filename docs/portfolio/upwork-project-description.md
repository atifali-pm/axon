...
- **MCP-native integrations.** Three MCP servers: `@axon/postgres-mcp` (read-only SQL, RLS-scoped), `@axon/github-mcp` (Octokit-backed, write-gated), and `@axon/custom-mcp` (template for new ones). Agents load MCP tools per tenant. Works with Claude Desktop too.
